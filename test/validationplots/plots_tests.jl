@testset "plots.jl" begin
    ...
    fm = MaximumLikelihoodEVA(BlockMaxima{GeneralizedExtremeValue}(Variable("y", y)), [1.0, 0.0, 0.1])

    x = rand(n)
    yns = rand.(GeneralizedExtremeValue.(x, 1.0, 0.1))
    fmns = MaximumLikelihoodEVA(BlockMaxima{GeneralizedExtremeValue}(Variable("y", yns), locationcov = [Variable("x", x)]), [0.0, 1.0, 0.0, 0.1])
