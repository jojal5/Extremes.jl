...
Several diagnostic plots for assessing the accuracy of the GEV model fitted to the Port Pirie data are can be shown with the `diagnosticplots` function of ExtremePlots.jl:
...
The inference for the fitted model is performed with the same methods defined previously for the GEV distribution, notably with the [`cint`](@ref) and [`returnlevel`](@ref) methods. Diagnostic plot methods `diagnosticplots` are also implemented, such as `probplot`, `qqplot`, `histplot` and `returnlevelplot`.
